...
- Use the local authenticator destination for vault-backed RFC 6238 TOTP code snapshots and optional independently credentialed toy-lock records. New entries and TOTP locks use an explicit 60-second local QR/manual pairing reveal and a current-code confirmation before vault storage. New locks use a fixed 20-target desktop catalog: the authenticator destination, 15 server-settings tabs, three appearance targets, and the authenticator-entry form. The app rejects free-form target identities, guards only those registered routes, and offers local plain-text/regular-expression lock discovery plus confirmation-gated per-record removal. QR image, clipboard, and camera import, every-rendered-element locking, broad context-menu or universal keyboard coverage, secret export, and synchronization remain unavailable.
