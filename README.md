...
- Inspect and deliberately change only the selected controlled server root's bounded operators, allowlist, player-ban, and IP-ban records. The desktop never resolves a player identity, sends a moderation command, uses RCON for these files, or claims a running server applied a disk change.
...
- Select a private local personal-vocabulary JSON payload through Studio preferences. The privileged local validator accepts only a bounded version-1 contract, retains only validated local data, revalidates it before use, and sends the renderer only a bounded final display-copy projection for a fixed safe allowlist—not raw replacement data or source metadata. History, exports, and status records remain separate. Clear returns the affected copy to original shipped wording; School mode hides the control and uses original wording while active.
...
- Configure a local appearance and navigation foundation with persisted system/light/dark theme, density, seed color, bounded typography, and seven canonical rendered profiles: app shell, settings-tab strip, primary and secondary actions, preference cards, status cards, and dialog surfaces. Each profile records color, shape, bounded typography, density, and motion overrides with inherited-value provenance; tab context actions can open the tab-strip profile. The current desktop window has a browser-style server tab workspace with docked accessible orientation, local order/pin/group persistence, overflow, separate current-strip/group/master searches with anchored regex builders, and protected bulk-close previews that exclude pinned and locked tabs by default. The bounded `Ctrl+Shift+F` palette can reveal real local targets; every-element editing, multi-window tab discovery, color-space translation, and complete command-palette coverage remain explicitly unavailable.
- Inspect the fixed local Ollama service through main-process-only reads for version, installed models, and running models. A freshly observed installed model can be re-pulled, copied to a bounded local name, or deleted through named fixed-loopback operations; delete is bound to the reusable two-confirmation/full-slider authorization. The destination keeps official catalog selection, batch pulls, chat, hardware-fit, and harness controls visibly unavailable, never sends requests to a cloud endpoint, and accepts no arbitrary shell command or generic API request.
- Optionally enable a local event narrator with independently selected English and Cantonese platform voices, rate and pitch controls, serialized speech, and an explicit browser screen-reader-detection limitation; add bounded app-private schema-version-2 scheduled **language** rules with local, validated main-process HTTPS API, or Home Assistant boolean sources. A matching HTTPS source can supply one validated language mode, while a Home Assistant `on` state activates a stored language rule; neither can set an appearance value. Credentials remain protected and runtime connection state remains unverified in this source-level foundation.
- Use the local authenticator destination for vault-backed RFC 6238 TOTP code snapshots and optional independently credentialed toy-lock records. New entries and TOTP locks use an explicit 60-second local QR/manual pairing reveal and a current-code confirmation before vault storage. New locks use a fixed 21-target desktop catalog: the authenticator destination, 16 server-settings tabs, three appearance targets, and the authenticator-entry form. The app rejects free-form target identities, guards only those registered routes, and offers local plain-text/regular-expression lock discovery plus confirmation-gated per-record removal. QR image, clipboard, and camera import, every-rendered-element locking, broad context-menu or universal keyboard coverage, secret export, and synchronization remain unavailable.
...
5. Edit the structured controls on the General, World, Gameplay, Network, Access records, Runtime, Paper JAR CLI, BuildTools, Live management, Command Center, Plugins, Console, and Local status tabs.
...
- Personal-vocabulary replacement data is validated and retained only locally. Raw replacement data, source metadata, cached data, and replacement evidence do not cross renderer IPC, local history, status, exports, logs, telemetry, or network boundaries; clearing the setting removes the validated local cache.
...
- External scheduled-language sources are configured and requested only by the Electron main process. A validated HTTPS result is bounded, versioned, and allowlisted to the three language modes; a Home Assistant boolean source can only activate its stored rule when its validated entity is `on`. A protected source token never enters the schedule file, renderer snapshot, local history, export, log, or public record. Missing configuration, invalid data, offline/network failures, an `off` entity, or a rejected response retains the local base or another valid matching rule rather than persisting a remote value.
...
- [Local server access records](docs/features/server-access-records.md)
...
- [Local Ollama suite](docs/features/local-ollama-suite.md)
