...
- Configure a local appearance and navigation foundation with persisted system/light/dark theme, density, seed color, bounded typography, direct shell/tab/action targets, and a browser-style server tab strip that docks on any edge, keeps keyboard orientation correct, persists selection, searches locally, and exposes an all-tabs overflow list. The bounded `Ctrl+Shift+F` palette can reveal real local targets, while every-element editing, tab pinning/grouping/reordering, and full command-palette coverage remain explicitly unavailable.
