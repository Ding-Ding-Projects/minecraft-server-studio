...
- Local notification-center foundation with non-blocking corner toasts, bounded scrubbed application-data history, user-opened review/filter/select/mark-read/dismiss controls, and explicit unavailable-history behavior.
- Shared destructive-action confirmation component with two independent confirmations, a full-range authorization slider, Emergency exit, Escape cancellation, reduced-motion completion behavior, focus restoration, and current-plan execution handoff for restore, Paper JAR, BuildTools, and Command Center routes.
