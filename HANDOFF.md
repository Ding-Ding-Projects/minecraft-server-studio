...
- `src/main/ollama-suite-manager.cjs`, `src/main/local-history-service.cjs`, `src/main/main.cjs`, `src/main/preload.cjs`, `src/main/server-manager.cjs`, and the Local Ollama renderer destination: fixed-loopback main-process version/installed/running-model reads plus named bounded selected-installed-model re-pull, copy, delete preview/delete, and cancellation routes. The manager validates all request/response bounds, rejects arbitrary endpoint/method/body selection, binds deletion to a short-lived two-key confirmation digest, and records only redacted operation outcomes locally.
...
- `docs/features/local-ollama-suite.md`: fixed-loopback local Ollama selected-model operations, disabled capability boundaries, recovery states, privacy constraints, redacted history behavior, and unrun verification boundary.
...
- Keep the fixed browser request allowlist and no-request-before-refresh
  ...
  boundary is lifted.
- Implement the full Ollama catalog, batch pull cart/progress/resume, chat,
  hardware-fit, and harness workflows only in separately scoped desktop work
  with their required security, offline, accessibility, and runtime proof.

## Local Ollama suite

The Local Ollama destination remains a deliberately narrow fixed-loopback desktop surface. Its main-process-only manager permits only `http://127.0.0.1:11434`, and only named request specifications can reach the documented version, installed-model, running-model, selected-model re-pull, copy, and delete endpoints. There is no renderer endpoint setting, generic request, token, cloud proxy, raw response body, prompt, chat response, or arbitrary shell command path.

The inventory remains bounded and searchable. One freshly observed installed model can be re-pulled with `stream: false`, copied to a bounded lowercase local name, or deleted after a short-lived two-key/full-slider confirmation whose authority is bound to the selected model. Mutations are serialized; cancellation aborts the app request and triggers a fresh bounded inventory read because the local service may already have completed work. Successful actions create redacted local-history events without model names or raw service data.

Official catalog selection, arbitrary pull input, batch cart/progress/resume, chat, hardware-fit decisions, and harness launch remain visible unavailable boundaries.

### Verification boundary

This source and documentation lane did not run tests, linting, type checks, independent review, build, package, runtime interaction, built-artifact interaction, or captures. It is not proof that Ollama is installed, that a local model can run, that a selected-model operation succeeds on a device, or that a catalog, batch pull, chat, or harness workflow exists.
...
- The local Ollama suite does not yet complete the exhaustive official Model Store, catalog pagination and revision evidence, arbitrary catalog selection, batch pulls with progress/resume, hardware-fit assessment, streamed chat, capability-gated attachments, registered harness profiles, rollback, offline documentation/troubleshooting, or their required proof set.
