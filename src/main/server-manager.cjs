...
const STATUS_COMPLETENESS_ROWS = Object.freeze({
  ...
  'console-and-rcon': { implementationPath: ['src/main/server-manager.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/rcon-response-safety.js', 'src/renderer/renderer.js', 'src/renderer/index.html'], documentationPath: ['docs/features/server-orchestration.md', 'docs/features/rcon-response-safety.md'], localization: { state: 'pending' }, test: { state: 'pending' }, capture: { state: 'pending' }, evidence: { state: 'in-progress', detail: 'Local console is available; desktop RCON response presentation now uses a redacted and bounded IPC envelope, while runtime verification remains pending.' } },
