...
const STATUS_COMPLETENESS_ROWS = Object.freeze({
  ...
  'ollama': { implementationPath: ['src/main/ollama-suite-manager.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/main/server-manager.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js'], documentationPath: ['docs/features/local-ollama-suite.md'], localization: { state: 'pending', detail: 'The local Ollama suite foundation is English-only; localization remains incomplete.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'Bounded main-process loopback health, installed-model, and running-model inventory source is registered. Official catalog pagination, pulls, chat, hardware fit, harness profiles, and runtime verification remain incomplete.' } },
