...
const STATUS_COMPLETENESS_ROWS = Object.freeze({
  ...
  'spigot-buildtools': { implementationPath: ['src/main/buildtools-adapter.cjs', 'src/main/buildtools-orchestration.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js'], documentationPath: ['docs/features/spigot-buildtools.md', 'docs/features/buildtools-orchestration.md'], localization: { state: 'pending', detail: 'BuildTools plan-only copy is English-only; localization remains incomplete.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'The legacy adapter and a separate typed plan-only controller are registered. The planner never starts a process, downloads BuildTools, creates a workspace, produces a JAR, or acquires a plugin; execution verification remains pending.' } },
  ...
  'server-access-records': { implementationPath: ['src/main/server-access-records.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/server-access-records.md', 'docs/features/server-orchestration.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'pending', detail: 'Access-record copy is English-only in this source foundation.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'Bounded local access-record source is registered for controlled server roots only. It validates and atomically writes operators, allowlist, player-ban, and IP-ban JSON records without shell, network, RCON, live command, player-identity lookup, or runtime-application claims.' } },
  ...
  'settings-appearance-and-localization': { implementationPath: ['src/main/appearance-navigation-settings.cjs', 'src/main/studio-settings.cjs', 'src/main/narration-schedule-settings.cjs', 'src/main/logo-manager.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/experience-copy.js', 'src/renderer/narrator.js', 'src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/appearance-and-tabs.md', 'docs/features/desktop-tab-workspace.md', 'docs/features/experience-settings.md', 'docs/features/app-logo-customization.md', 'docs/features/narrator-and-scheduled-settings.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'in-progress', detail: 'The language-mode, message-style, display-name, dialog-emoji, School-mode, narrator, scheduled-language, bounded appearance/tab-navigation, and app-logo foundations are registered; complete application-wide localization remains pending.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'App-private settings, a watched shared local record, protected unlock storage, local narrator preferences, renderer speech queue wiring, bounded local language schedules, persisted M3 theme/density/seed/typography controls, seven canonical rendered appearance profiles with inherited-value provenance and tab-context routing, and one-window persisted desktop tab-workspace source for order, pins, groups, collapse, searches, overflow, and confirmation-routed close actions are registered. Platform speech, external schedule adapters, every-element appearance editing, color-space translation, multi-window tab discovery, logo runtime conversion, localization, and universal-surface requirements remain incomplete.' } },
  'personal-vocabulary-upload': { implementationPath: ['src/main/personal-vocabulary-copy-catalog.cjs', 'src/main/personal-vocabulary-manager.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/experience-copy.js', 'src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/personal-vocabulary-upload.md', 'docs/features/experience-settings.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'in-progress', detail: 'The local settings card uses the existing English, Cantonese, bilingual, and shared-mode copy path; complete application-wide localization remains pending.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'A bounded main-process JSON validator rejects unsupported schema versions, duplicate or unsafe keys, unexpected fields, excessive nesting, and over-limit text before promoting an app-private cache. While the shared mode is inactive, the renderer receives only a bounded final display-copy projection for a fixed safe allowlist, never replacement entries. The source name, location, cache payload, and file metadata are excluded from history, exports, and local status; runtime interaction and complete surface coverage remain pending.' } },
  'command-palette': { implementationPath: ['src/main/desktop-status-model.cjs', 'src/main/server-manager.cjs', 'src/renderer/experience-copy.js', 'src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/desktop-command-palette.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'in-progress', detail: 'The palette framing follows the existing language-mode copy route; complete result-catalog localization remains pending.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'A local Ctrl+Shift+F palette source indexes known renderer destinations, current tabs, loaded bundled-document records, and non-secret controls. It uses bounded local plain-text or regex search, exact-target focus, and the existing update-check change handler. Secret indexing, arbitrary execution, new IPC, full catalog coverage, and runtime verification are not claimed.' } },
  'application-updates': { implementationPath: ['src/main/update-controller.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js'], documentationPath: ['docs/features/unsigned-automatic-updates.md'], localization: { state: 'pending', detail: 'Application-update copy is English-only; localization remains incomplete.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'Unsigned Squirrel.Windows application-update source now derives its installed version only from packaged Electron metadata, validates stable numeric full-package versions independently of release tags and RELEASES row order, and refuses rollback packages; package and runtime verification remain pending.' } },
  'file-converter': { implementationPath: ['src/main/file-converter.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/file-converter.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'pending', detail: 'The converter source currently has English-only copy; application-wide localization remains pending.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'A local-only bounded native source picker, in-process UTF-8/JSON/CSV/TSV/YAML-style/Base64/hex conversion source, user-approved new destination path, atomic output validator, safe result records, unavailable-adapter catalog, and renderer destination are registered. Packaged execution, conversion interaction, localization, focused tests, and capture proof remain pending; PDF/media/archive/XML/native-workbook conversion is not claimed.' } },
  'dim-sum-surprise': { implementationPath: ['src/main/dim-sum-surprise.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/experience-copy.js', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/dim-sum-surprise.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'in-progress', detail: 'The displayed dish name/text alternative and independently funny-levelled English/Cantonese surprise heading select through the existing language modes; complete surprise-surface localization remains pending.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'The main-process-only public-catalog cache, exact 10% fresh draw, first-run/update/School-mode/error/draft/modal suppression, bounded PNG revalidation, non-blocking renderer/narrator source, and reduced-motion source are registered. Runtime cache exercise, distribution proof, assistive-technology verification, localization breadth, and captures remain pending.' } },
  'ollama': { implementationPath: ['src/main/ollama-suite-manager.cjs', 'src/main/local-history-service.cjs', 'src/main/offline-docs.cjs', 'src/main/desktop-status-model.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/main/server-manager.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/local-ollama-suite.md', 'docs/features/local-history-and-safe-exports.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'pending', detail: 'The local Ollama suite foundation is English-only; localization remains incomplete.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'Bounded main-process loopback health and installed/running inventory source is registered alongside named selected-installed-model re-pull, copy, delete-preview/delete, and cancellation IPC. The fixed operation table validates request/response bounds, blocks arbitrary endpoint or method selection, binds deletion to a short-lived two-key confirmation digest, and records only redacted operation metadata locally. Official catalog pagination, arbitrary pull selection, batch cart/progress/resume, chat, hardware fit, harness profiles, localization, and runtime verification remain incomplete.' } },
  'docs-history-and-notifications': { implementationPath: ['src/main/offline-docs.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/markdown-renderer.js', 'src/renderer/index.html', 'src/renderer/renderer.js'], documentationPath: ['docs/features/offline-documentation-browser.md', 'docs/features/README.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'pending', detail: 'Offline documentation copy is English-only; complete localization remains pending.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'A fixed app-bundled feature-document inventory, narrow reader IPC, escaped Markdown-renderer seam, internal article-link route, and documentation search source are registered. Local history and complete localization/runtime evidence remain pending; notification-center evidence is tracked independently.' } },
  'notifications-and-destructive-confirmation': { implementationPath: ['src/main/notification-center-service.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/desktop-notifications-and-destructive-confirmation.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'pending', detail: 'Notification-center and destructive-confirmation copy remains English-only in this foundation.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'A bounded app-private safe-summary notification store, narrow IPC, persistent corner-toast behavior, local center with individual/bounded bulk actions, and a reusable two-control/full-slider confirmation surface are registered. Complete localization, interaction coverage, packaged-runtime proof, and captures remain pending.' } },
  'changelog-viewer': { implementationPath: ['src/main/changelog-library.cjs', 'src/main/release-catalog.json', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/experience-copy.js', 'src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/changelog-viewer.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'in-progress', detail: 'Navigation and framing use the existing English, Cantonese, and bilingual copy route; complete destination localization remains pending.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'A fixed bundled changelog parser, retained checked-in release metadata baseline, package-generated local-tag snapshot, narrow renderer IPC, local date/plain-text/regex filtering, copy/export path, and user-initiated validated commit handoff are registered. Remote release fetching, runtime interaction, and full release-history proof are not claimed.' } },
  'authenticator-and-toy-locks': { implementationPath: ['src/main/totp-engine.cjs', 'src/main/authenticator-service.cjs', 'src/main/totp-pairing-service.cjs', 'src/main/toy-lock-service.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/index.html', 'src/renderer/totp-qr.js', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/authenticator-and-toy-locks.md'], localization: { state: 'pending', detail: 'Authenticator and toy-lock copy is English-only in this foundation.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'A bounded 20-target catalog covers the authenticator destination, 15 server settings tabs, three appearance targets, and the authenticator-entry form. The main process rejects unknown targets and label mismatches; renderer registration, direct configuration routes, guarded target paths, plain-text/regex metadata discovery, and confirmation-gated removal are registered. Local RFC 6238 code generation, protected-vault secret storage, bounded manual Base32 and standard otpauth URI parsing, canonical URI generation, safe code-only IPC snapshots, and a 60-second in-process QR/manual pairing reveal with current-code confirmation now gate a new entry or TOTP toy lock. QR-image, clipboard, and camera import, every-rendered-element/context-menu/keyboard coverage, bulk actions, localization, focused checks, runtime/package interaction, and captures remain incomplete.' } },
  'support-tickets': { implementationPath: ['src/main/support-ticket-service.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/support-tickets.md'], localization: { state: 'pending', detail: 'Support Tickets copy is English-only in this foundation.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'A bounded local-only ticket record, renderer destination, regex-enabled ticket search, canned status progression, and a user-initiated exact application-data-folder handoff are registered. No network delivery, human support service, automated deletion, runtime interaction, or broad localization is claimed.' } },
  'local-history-and-safe-exports': { implementationPath: ['src/main/local-history-service.cjs', 'src/main/offline-docs.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js'], documentationPath: ['docs/features/local-history-and-safe-exports.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'pending', detail: 'The local history and export foundation is English-only; localization remains incomplete.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'A bounded redacted JSONL journal, date/action/plain-text/regex filtering, offline-documentation inventory entry, structured safe exports, and a verified-output-only VS Code handoff are registered. No Git repository, snapshots, restoration, or runtime verification is claimed.' } },
  'external-editor-handoff': { implementationPath: ['src/main/external-editor-service.cjs', 'src/main/local-history-service.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/external-editor-integration.md', 'docs/features/local-history-and-safe-exports.md'], localization: { state: 'pending', detail: 'External-editor copy is English-only in this foundation.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'A shared bounded local Visual Studio Code detector and app-private selection record are registered. Direct shell-free handoff can target a validated selected server root, a path-redacted generated record, or a verified app-private history export. Runtime interaction and complete export coverage remain pending.' } },
  'export': { implementationPath: ['src/main/external-editor-service.cjs', 'src/main/local-history-service.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js'], documentationPath: ['docs/features/external-editor-integration.md', 'docs/features/local-history-and-safe-exports.md'], localization: { state: 'pending' }, test: { state: 'pending' }, capture: { state: 'pending' }, evidence: { state: 'in-progress', detail: 'A narrow structured history export and shared local external-editor handoff are registered. Complete structured exports remain pending.' } }
});
