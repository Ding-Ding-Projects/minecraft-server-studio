...
const STATUS_COMPLETENESS_ROWS = Object.freeze({
  ...
  'backups-and-updates': { implementationPath: ['src/main/update-controller.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js'], documentationPath: ['docs/features/unsigned-automatic-updates.md', 'docs/features/server-orchestration.md'], localization: { state: 'pending', detail: 'Application-update copy is English-only; localization remains incomplete.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'Unsigned Squirrel.Windows application-update source is registered. Backup-first server, plugin, and world update/rollback controls remain pending.' } },
