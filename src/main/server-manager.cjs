...
const STATUS_COMPLETENESS_ROWS = Object.freeze({
  ...
  'file-converter': { implementationPath: ['src/main/file-converter.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/index.html', 'src/renderer/renderer.js'], documentationPath: ['docs/features/file-converter.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'pending', detail: 'The converter foundation currently has English-only source copy; application-wide localization remains pending.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'A local-only bounded byte-inspection queue, disabled adapter catalog, native source picker, and renderer destination are registered. No output adapter, conversion result, package proof, or runtime verification is claimed.' } },
