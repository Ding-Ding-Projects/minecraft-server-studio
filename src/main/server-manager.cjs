...
const STATUS_COMPLETENESS_ROWS = Object.freeze({
  ...
  'settings-appearance-and-localization': { implementationPath: ['src/main/appearance-navigation-settings.cjs', 'src/main/studio-settings.cjs', 'src/main/narration-schedule-settings.cjs', 'src/main/main.cjs', 'src/main/preload.cjs', 'src/renderer/experience-copy.js', 'src/renderer/narrator.js', 'src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/styles.css'], documentationPath: ['docs/features/appearance-and-tabs.md', 'docs/features/experience-settings.md', 'docs/features/narrator-and-scheduled-settings.md', 'docs/features/local-status-and-completeness.md'], localization: { state: 'in-progress', detail: 'The language-mode, message-style, display-name, dialog-emoji, School-mode, narrator, scheduled-language, and bounded appearance/tab-navigation foundations are registered; complete application-wide localization remains pending.' }, test: { state: 'pending', detail: 'No test was run in this delivery pass.' }, capture: { state: 'pending', detail: 'No built-artifact capture was run in this delivery pass.' }, evidence: { state: 'in-progress', detail: 'App-private settings, a watched shared local record, protected unlock storage, local narrator preferences, renderer speech queue wiring, bounded local language schedules, persisted M3 theme/density/seed/typography controls, direct appearance targets, and dockable tab selection/overflow source are registered. Platform speech, external schedule adapters, every-element appearance editing, tab grouping or pinning, localization, and universal-surface requirements remain incomplete.' } },
